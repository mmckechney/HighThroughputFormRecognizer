...
namespace FormProcessorFunction
{
    public class Recognition
    {
        ...
        [FunctionName("Recognition")]
        public async Task Run([ServiceBusTrigger("formqueue", AutoCompleteMessages = true)] ServiceBusReceivedMessage message, ILogger log)
        {
            logger = log;
            try
            {
        ...
        public async Task<string> ProcessFormRecognition(Uri fileUri, int index)
        {
            ...
            try
            {
                ...
                {
                    operation = await formRecogClient.StartAnalyzeDocumentFromUriAsync(Settings.DocumentProcessingModel, fileUri);
                }, source.Token);
